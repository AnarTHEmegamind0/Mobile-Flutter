...
class _BagPageState extends State<BagPage> {
  ...
  Future<List<ProductModel>> _getCartData() async {
    List<CartModel> data =
        await Provider.of<MyRepo>(context, listen: false).fetchCartData();

    if (data.isNotEmpty) {
      final hello = await Provider.of<MyRepo>(
        context,
        listen: false,
      ).getCartItems(context, data);

      Provider.of<globalProvider>(context, listen: false).setCartItems(hello);
      return hello;
    } else {
      Provider.of<globalProvider>(context, listen: false).setCartItems([]);
      return [];
    }
  }


  double getTotal(List<ProductModel> data) {
    double total = 0;
  ...
  double _calculateTotal(List<ProductModel> items) {
    return items.fold(0, (sum, item) => sum + (item.price ?? 0.0));
  }
